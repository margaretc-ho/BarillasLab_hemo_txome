...
### v7 keeps the same consensus transcriptome, but adds transdecoder analysis (including uniprot and pfam) prediction for 2907 transcripts
### that were not predicted as protein coding by CodAn
### and that were not aligned to any known AGAP genes
...
mkdir -p ${v7_dir}/notAGAPnotCodAn

#grep -w "transcript" ${v7_dir}/notAGAPnotCodAn/ref_guided_idcollapse.strandfiltered.notAGAPnotCodAn.gtf | cut -f 9 | cut -f 2 -d " " | sed s'/"//g' | sed s'/;//g' > ${v7_dir}/notAGAPnotCodAn/notAGAPnotCodAn_transcripts.txt

#grep -v -f ${v7_dir}/Agamhemo_codAn_partial/TCONS_of_proteincodinggenes_alltx.fa.txt  ${Agamhemo_strandfilteredtranscripts_gtf} |  grep -v AGAP > ${v7_dir}/notAGAPnotCodAn/ref_guided_idcollapse.strandfiltered.notAGAPnotCodAn.gtf
...
grep -f ${v7_dir}/notAGAPnotCodAn/notAGAPnotCodAn_transcripts.txt ${classcodes} | cut -f 1,3 > ${v7_dir}/notAGAPnotCodAn/notAGAPnotCodAn_transcripts.classcodes.txt

#gffread -w ${v7_dir}/notAGAPnotCodAn/notAGAPnotCodAn_transcripts.comprehensive.v6.gffread.fa -g ${reference_genome} ${v7_dir}/notAGAPnotCodAn/ref_guided_idcollapse.strandfiltered.notAGAPnotCodAn.gtf

### Trinotate ### rerunning to compare with CodAn

# module purge
# module load transdecoder
#TransDecoder.LongOrfs -S -t ${v7_dir}/notAGAPnotCodAn/notAGAPnotCodAn_transcripts.comprehensive.v6.gffread.fa --output_dir ${v7_dir}/notAGAPnotCodAn/transdecoder
...
#blastp -query ${v7_dir}/notAGAPnotCodAn/transdecoder/longest_orfs.pep -db ${uniprotdb} -num_threads 16 -max_target_seqs 1 -outfmt 6 > ${v7_dir}/notAGAPnotCodAn/transdecoder/blastp.outfmt6

#### Running PFAM HMMER
# hmmsearch --cpu 16 -E 1e-10 --domtblout ${v7_dir}/notAGAPnotCodAn/transdecoder/pfam.domtblout ${pfamhmm} ${v7_dir}/notAGAPnotCodAn/transdecoder/longest_orfs.pep > ${v7_dir}/notAGAPnotCodAn/transdecoder/pfam.log
...
# TransDecoder.Predict --single_best_only -t ${v7_dir}/notAGAPnotCodAn/notAGAPnotCodAn_transcripts.comprehensive.v6.gffread.fa --retain_pfam_hits ${v7_dir}/notAGAPnotCodAn/transdecoder/pfam.domtblout --retain_blastp_hits ${v7_dir}/notAGAPnotCodAn/transdecoder/blastp.outfmt6 --output_dir ${v7_dir}/notAGAPnotCodAn/transdecoder/